...
- src/views/Agents.tsx — add a model assignment badge on each agent list item. Color-code: amber-500 text for MAX tier (models starting with "anthropic/"), emerald-500 text for free tier (models starting with "copilot/" or "google/"). Show the primary model name in a small text-xs badge below the agent name. Show the fallback chain in a tooltip or expandable section. When an agent is selected, add a "Role" section in the detail panel that displays the first ~5 lines of the agent'\''s soul.md file content (already available via the file browser — check if the selected file is soul.md).
...
YOUR FILES (only touch these):
- src/views/Models.tsx — enhance the existing model list. Group models by provider (use the provider field on ModelChoice). For each model, add a "Cost" display: show "$0" with emerald-500 text for models from copilot/* and google/* providers, show the actual cost data for others. Add an "Assigned Demons" column/section showing emoji badges of demons that use this model as their primary (cross-reference the agents store — read agent.model.primary). Add an emerald "Free" badge next to free-tier model names.
